...
def find_duplicate_groups(
    items: list[tuple[int, int, str, str, str, int | None]],
    *,
    gallery_titles: dict[int, tuple[str | None, str | None]],
) -> list[dict[str, Any]]:
    """Group favorite items that are likely the same work in different versions.

    ``items`` is ``(favcat, gid, token, title, url, gallery_id)``.  Titles are
    taken from the favorite record, falling back to the local gallery's
    English then Japanese title.  Items with no usable title are skipped.
    ...
    for favcat, gid, token, title, url, gallery_id in items:
        eff_title = title
