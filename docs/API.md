...
```json
{
  ...
  "image_slow_warmup_seconds": 10,
  "image_min_speed_kb_s": 50,
...
| POST | `/api/galleries/delete-bulk` | Body `{ids: [...], delete_files?: bool}`. Bulk remove galleries by id; `delete_files` also deletes on-disk files, keeping each row whose files failed to delete. Returns `{deleted, failed_deletions}`. |
